...
chmod 600 ~/.analyst-runtime/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.analyst-runtime/config.json` with file permissions set to `0600`
...
- ✅ Never run analyst_runtime as root
...
- ✅ Run analyst_runtime with a dedicated user account
...
- Keep authentication data in `~/.analyst-runtime/whatsapp-auth` secure (mode 0700)
...
pip install --upgrade analyst-runtime
```
...
- Subscribe to security advisories for Analyst Runtime and its dependencies
...
1. **Isolate the Environment**
   ...
   pip install analyst-runtime
   ```

2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash analyst_runtime
   sudo -u analyst_runtime analyst-runtime gateway
   ```

3. **Set Proper Permissions**
   ```bash
   chmod 700 ~/.analyst-runtime
   chmod 600 ~/.analyst-runtime/config.json
   chmod 700 ~/.analyst-runtime/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.analyst-runtime/logs/analyst_runtime.log
   ```
...
- **Chat history is stored locally** - protect the `~/.analyst-runtime` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.analyst-runtime/logs/analyst_runtime.log
   ```
...
Before deploying analyst_runtime:
