...
Analyst Runtime 的 tools 子系统，受 Runtime profile 和工具权限约束。它属于 **Runtime tools** 边界。当前目录不替代父级架构说明，也不接管其他 Module 的数据或安全决策。
...
| [`adapters.yaml`](<adapters.yaml>) | 本目录的主要实现、配置或受控资料 |
| [`analyze_image_tool.py`](<analyze_image_tool.py>) | 本目录的主要实现、配置或受控资料 |
| [`base.py`](<base.py>) | 本目录的主要实现、配置或受控资料 |
| [`composio.py`](<composio.py>) | 本目录的主要实现、配置或受控资料 |
| [`cron.py`](<cron.py>) | 本目录的主要实现、配置或受控资料 |
| [`filesystem.py`](<filesystem.py>) | 本目录的主要实现、配置或受控资料 |
| [`firecrawl.py`](<firecrawl.py>) | 本目录的主要实现、配置或受控资料 |
