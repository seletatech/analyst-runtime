...
Analyst Runtime 的 channels 子系统，受 Runtime profile 和工具权限约束。它属于 **Runtime channels** 边界。当前目录不替代父级架构说明，也不接管其他 Module 的数据或安全决策。
...
| [`dingtalk.py`](<dingtalk.py>) | 本目录的主要实现、配置或受控资料 |
| [`discord.py`](<discord.py>) | 本目录的主要实现、配置或受控资料 |
| [`email.py`](<email.py>) | 本目录的主要实现、配置或受控资料 |
| [`feishu.py`](<feishu.py>) | 本目录的主要实现、配置或受控资料 |
| [`manager.py`](<manager.py>) | 本目录的主要实现、配置或受控资料 |
| [`mochat.py`](<mochat.py>) | 本目录的主要实现、配置或受控资料 |
