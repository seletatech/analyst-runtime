...
Analyst Runtime 的 config 子系统，受 Runtime profile 和工具权限约束。它属于 **Runtime config** 边界。当前目录不替代父级架构说明，也不接管其他 Module 的数据或安全决策。
...
| [`loader.py`](<loader.py>) | 本目录的主要实现、配置或受控资料 |
| [`schema.py`](<schema.py>) | 本目录的主要实现、配置或受控资料 |
