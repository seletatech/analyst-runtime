...
class TelegramConfig(Base):
    ...
    send_only: bool = False  # skip polling; inbound arrives via gateway webhook
...
class WebConfig(Base):
    ...
    gateway_url: str = "http://host.docker.internal:8000"  # Gateway HTTP base URL
    gateway_jwt_token: str = ""  # Shared secret for internal gateway auth
    allow_from: list[str] = Field(default_factory=lambda: ["*"])  # Allowed session IDs ("*" = allow all)
...
class AgentDefaults(Base):
    ...
    workspace: str = "~/.analyst-runtime/workspace"
...
class Config(BaseSettings):
    """Root configuration for analyst_runtime."""
    ...
    model_config = ConfigDict(env_prefix="ANALYST_RUNTIME_", env_nested_delimiter="__")
