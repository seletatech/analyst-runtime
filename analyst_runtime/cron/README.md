...
Analyst Runtime 的 cron 子系统，受 Runtime profile 和工具权限约束。它属于 **Runtime cron** 边界。当前目录不替代父级架构说明，也不接管其他 Module 的数据或安全决策。
...
| [`service.py`](<service.py>) | 本目录的主要实现、配置或受控资料 |
...
This directory owns scheduled-task support. If more schedulers or persistence layers appear, split scheduling policy from execution mechanics.

## What Lives Here
- `__init__.py` - current entrypoint or supporting file in this directory
- `service.py` - current entrypoint or supporting file in this directory
- `types.py` - current entrypoint or supporting file in this directory
