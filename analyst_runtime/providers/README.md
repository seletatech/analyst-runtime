...
Analyst Runtime 的 providers 子系统，受 Runtime profile 和工具权限约束。它属于 **Runtime providers** 边界。当前目录不替代父级架构说明，也不接管其他 Module 的数据或安全决策。
...
| [`base.py`](<base.py>) | 本目录的主要实现、配置或受控资料 |
| [`custom_provider.py`](<custom_provider.py>) | 本目录的主要实现、配置或受控资料 |
| [`litellm_provider.py`](<litellm_provider.py>) | 本目录的主要实现、配置或受控资料 |
| [`openai_codex_provider.py`](<openai_codex_provider.py>) | 本目录的主要实现、配置或受控资料 |
| [`registry.py`](<registry.py>) | 本目录的主要实现、配置或受控资料 |
| [`transcription.py`](<transcription.py>) | 本目录的主要实现、配置或受控资料 |
| [`tts.py`](<tts.py>) | 本目录的主要实现、配置或受控资料 |
