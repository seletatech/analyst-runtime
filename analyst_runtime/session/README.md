...
Analyst Runtime 的 session 子系统，受 Runtime profile 和工具权限约束。它属于 **Runtime session** 边界。当前目录不替代父级架构说明，也不接管其他 Module 的数据或安全决策。
...
This directory manages long-lived runtime session state. If state management keeps growing, introducing clearer submodules would help.
