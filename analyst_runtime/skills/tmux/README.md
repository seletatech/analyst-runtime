...
Analyst Runtime 可发现的 `tmux` 技能包；SKILL.md 是行为入口。它属于 **tmux skill** 边界。当前目录不替代父级架构说明，也不接管其他 Module 的数据或安全决策。
...
| 继续查找实现或资料 | 进入 [`scripts/`](<scripts/README.md>) |
...
| 子目录 | 内容 | 什么时候进入 |
|---|---|---|
| [`scripts/`](<scripts/README.md>) | Analyst Runtime 可发现的 `scripts` 技能包；SKILL.md 是行为入口 | 修改或核验该职责时 |
