...
Analyst Runtime 可发现的 `scripts` 技能包；SKILL.md 是行为入口。它属于 **scripts skill** 边界。当前目录不替代父级架构说明，也不接管其他 Module 的数据或安全决策。
...
| 你要做的事情 | 下一步 |
|---|---|
| 查看主要入口或配置 | 打开 [`find-sessions.sh`](<find-sessions.sh>) |
...
| 文件 | 用途 |
|---|---|
| [`find-sessions.sh`](<find-sessions.sh>) | 本目录的主要实现、配置或受控资料 |
| [`wait-for-text.sh`](<wait-for-text.sh>) | 本目录的主要实现、配置或受控资料 |
