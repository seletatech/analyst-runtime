...
Analyst Runtime 的 utils 子系统，受 Runtime profile 和工具权限约束。它属于 **Runtime utils** 边界。当前目录不替代父级架构说明，也不接管其他 Module 的数据或安全决策。
...
| [`helpers.py`](<helpers.py>) | 本目录的主要实现、配置或受控资料 |
| [`tool_calls.py`](<tool_calls.py>) | 本目录的主要实现、配置或受控资料 |
...
This directory should stay tiny and only contain helpers that are truly runtime-wide.

## What Lives Here
- `__init__.py` - current entrypoint or supporting file in this directory
- `helpers.py` - current entrypoint or supporting file in this directory
