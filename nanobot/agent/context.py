...
class ContextBuilder:
    ...
    def __init__(
        ...
        self.minimal = (
            os.environ.get("NANOBOT_MINIMAL_WORKSPACE", "").casefold()
            in {"1", "true", "yes"}
            if minimal is None
            else minimal
        )
...
For scheduled reminders, use the 'cron' tool directly. Do not call shell commands like `nanobot cron ...` via exec.
