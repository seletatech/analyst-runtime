"""Web channel: bridges FastAPI gateway to Nanobot internal bus via HTTP polling.

Message flow:
  FastAPI WebSocket -> asyncio.Queue (gateway process)
      <- GET /internal/sandbox/{id}/inbound  (Nanobot long-polls)
  Nanobot AgentLoop
      -> POST /internal/sandbox/{id}/outbound (Nanobot posts reply)
...
# ---------------------------------------------------------------------------
# WebChannel  (Nanobot BaseChannel implementation)
# ---------------------------------------------------------------------------

class WebChannel(BaseChannel):
    """Bridge between the FastAPI gateway and Nanobot's internal bus.
    ...
    async def send(self, msg: OutboundMessage) -> dict[str, Any]:
        ...
        if not self.sandbox_id:
            ...
            raise RuntimeError("NANOBOT-DELIVERY-001: Web channel is not configured")
        ...
        async with httpx.AsyncClient(trust_env=False) as client:
            for raw_path in media_paths:
                ...
                if not path.is_relative_to(workspace):
                    raise PermissionError("Attachments must be inside the NanoBot workspace")
                if not path.is_file():
                ...
                response = await client.post(
                    ...
                    headers={
                        ...
                        "X-Nanobot-Run-Id": run_id,
                        "X-Nanobot-Filename": quote(path.name),
                    },
                    timeout=30.0,
                )
    ...
    async def _process_inbound(self, payload: dict) -> None:
        """Convert a gateway payload to an InboundMessage and publish to Nanobot bus."""
