...
class WhatsAppChannel(BaseChannel):
    ...
    name = "whatsapp"
    _LINK_TOKEN_RE = re.compile(r"(NANOBOT-[A-Za-z0-9_-]+)")
    ...
    async def start_bridge(self) -> bool:
        ...
        for candidate in [Path.home() / ".nanobot" / "bridge", Path("/app/bridge")]:
    ...
    def _extract_link_token(cls, content: str) -> str | None:
        """Extract a NANOBOT one-time link token from free-form text."""
        if not content:
            return None
