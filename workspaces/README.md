...
`workspace/`. Keep the template directories immutable; NanoBot writes memory
and session state into the active workspace at runtime.

```bash
cp nanobot/workspaces/data-analyst/{SOUL.md,AGENTS.md,workspace.json} workspace/
```
...
- `workspace.json` enables optional runtime profiles. An absent or empty list
  means the generic NanoBot runtime only.

Choose runtime profiles when a product workspace is created. If a profile has
already persisted owned state, NanoBot fails closed when that profile is
...
is absent, NanoBot rejects all such metadata; the normal `SOUL.md` and
